...
int ICM45686_Initialization(void)
{
    ...
    // Reset por software mediante PWR_MGMT0
    ...
    // STATIC5 ACCEL → 0x0028
    if (ICM45686_WriteByte(ICM45686_REG_IREG_ADDR_15_8, 0x00) != ESP_OK) return 0;
    if (ICM45686_WriteByte(ICM45686_REG_IREG_ADDR_7_0,  0x28) != ESP_OK) return 0;
    if (ICM45686_WriteByte(ICM45686_REG_IREG_DATA,      0x00) != ESP_OK) return 0;
    vTaskDelay(pdMS_TO_TICKS(20));

    // STATIC5 GYRO → 0x0021
    if (ICM45686_WriteByte(ICM45686_REG_IREG_ADDR_15_8, 0x00) != ESP_OK) return 0;
    if (ICM45686_WriteByte(ICM45686_REG_IREG_ADDR_7_0,  0x21) != ESP_OK) return 0;
    if (ICM45686_WriteByte(ICM45686_REG_IREG_DATA,      0x00) != ESP_OK) return 0;
    vTaskDelay(pdMS_TO_TICKS(20));

    // LOW NOISE → activa 32 kHz internos
    if (ICM45686_WriteByte(ICM45686_REG_PWR_MGMT0, 0x0F) != ESP_OK)
        return 0;
    vTaskDelay(pdMS_TO_TICKS(20));

    // GYRO_CONFIG0: FS + ODR
    uint8_t gyro_cfg = ICM45686_GYRO_4000_DPS | ICM45686_GYRO_ODR_6_4KHz;
    if (ICM45686_WriteByte(ICM45686_REG_GYRO_CONFIG0, gyro_cfg) != ESP_OK)
        return 0;

    vTaskDelay(pdMS_TO_TICKS(20));

    // ACCEL_CONFIG0: FS + ODR
    uint8_t accel_cfg = ICM45686_ACCEL_16G | ICM45686_ACCEL_ODR_6_4KHz;
    ...
    // Filtros OFF (bypass)
    if (ICM45686_WriteByte(ICM45686_REG_ACCEL_CONFIG1, 0x00) != ESP_OK) return 0;
    if (ICM45686_WriteByte(ICM45686_REG_GYRO_CONFIG1,  0x00) != ESP_OK) return 0;

    ESP_LOGI("ICM", "Initialization OK at 6.4 kHz ODR (ADC interno 32 kHz)");
    return 1;
}
...
int ICM45686_Initialization_UF(void)
{
    ...
    // ================================================================
    // STATIC5 (ACELERÓMETRO) → 0x0028  :  BYPASS
    // ================================================================
    ICM45686_WriteByte(ICM45686_REG_IREG_ADDR_15_8, 0x00);
    ICM45686_WriteByte(ICM45686_REG_IREG_ADDR_7_0, 0x28);
    ICM45686_WriteByte(ICM45686_REG_IREG_DATA, 0x00);
    vTaskDelay(2);

    // ================================================================
    // STATIC5 (GIROSCOPIO) → 0x0021  :  BYPASS
    // ================================================================
    ICM45686_WriteByte(ICM45686_REG_IREG_ADDR_15_8, 0x00);
    ...
    // ================================================================
    // CONFIGURAR GIROSCOPIO (Máximo rendimiento)
    // FS = 4000 dps
    // ODR = 6.4 kHz
    // BW = Máx (473 Hz) → bits [7:4] = 0x0
    // ================================================================
    uint8_t gyro_cfg =
        ICM45686_GYRO_4000_DPS |
        ICM45686_GYRO_ODR_6_4KHz |
        (0x00 << 4);    // BW = máxima
    if (ICM45686_WriteByte(ICM45686_REG_GYRO_CONFIG0, gyro_cfg) != ESP_OK)
        return 0;
    ...
    // ================================================================
    // CONFIGURAR ACELERÓMETRO (Máximo rendimiento)
    // FS = ±16g
    // ODR = 6.4 kHz
    // BW = máxima
    // ================================================================
    ...
    // ================================================================
    // DESACTIVAR FILTROS DIGITALES UI
    // ACCEL_CONFIG1 = 0x00 → Bypass
    // GYRO_CONFIG1  = 0x00 → Bypass
    // ================================================================
    ...
    // ================================================================
    // ENCENDER ADCs DE 32 kHz (LOW NOISE)
    // bitfield:
    // [3:2] = Gyro mode → 0b11 LOW NOISE
    // [1:0] = Accel mode → 0b11 LOW NOISE
    // ================================================================
    if (ICM45686_WriteByte(ICM45686_REG_PWR_MGMT0, 0x0F) != ESP_OK)
        return 0;
    ...
    ESP_LOGI("ICM", "ICM45686 inicializado en modo ULTRA-FAST (6.4kHz + BW Máx)");
     ICM45686_RegisterDump_Corrected();
    return 1;
}
